  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 sm:p-8 font-sans text-slate-900 relative">
      ...
      <div className="relative z-10 w-full max-w-2xl animate-fade-in-up my-6 sm:my-10 px-0 sm:px-4">
        ...
        <div className="bg-white/80 border border-white/60 rounded-[1.5rem] sm:rounded-[2rem] p-6 sm:p-12 backdrop-blur-2xl shadow-[0_8px_32px_rgba(0,0,0,0.06)] relative">
          ...
          <button
            ...
            disabled={!isReady}
            className={`w-full py-4 sm:py-5 rounded-xl sm:rounded-2xl font-black text-lg sm:text-xl tracking-tight transition-all duration-300 ${isReady ? 'bg-slate-900 text-white shadow-xl hover:bg-slate-800 active:scale-[0.98] cursor-pointer' : 'bg-slate-100 text-slate-300 cursor-not-allowed border border-slate-100'}`}
          >
            Start Verification
          </button>
          {!isReady && (
